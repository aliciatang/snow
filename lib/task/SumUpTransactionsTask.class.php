    ...
    foreach($accounts as $account)
    {
      ...
      foreach($account->Transactions as $tran)
      {
        if($tran->security_id != $cur_sec)
        {
          ...
          $total_buy_amount = ($tran->quantity > 0 && $tran->action_id != 17)?$tran->amount:0;
          $total_buy_quantity = ($tran->quantity > 0 && $tran->action_id != 17)?$tran->quantity:0;
          $total_sell_amount = ($tran->quantity < 0 && $tran->action_id != 18)?$tran->amount:0;
          $total_sell_quantity = ($tran->quantity < 0 && $tran->action_id != 18)?$tran->quantity:0;
